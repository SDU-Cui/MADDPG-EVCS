...
if __name__ == "__main__":
    ...
    NET_CONFIG = {
        "arch": "mlp",  # 使用 MLP 作为网络架构
        "hidden_size": [128, 128],  # 两层隐藏层，每层 128 个神经元
        "mlp_activation": "ReLU",  # 隐藏层激活函数
        "mlp_output_activation": "Tanh",  # 输出层激活函数（适用于连续动作）
    }
    ...
    INIT_HP = {
        ...
        "O_U_NOISE": 0.1,  # 添加 OU 噪声
        "EXPL_NOISE": 0.01, #高斯噪声的标准差
        "MEAN_NOISE": 0.0,
        "THETA": 0.15, # OU噪声恢复强度
        "DT": 0.01, #时间步长
        ...
        for agent in pop:  # Loop through population
            ...
            for _ in range(max_steps):
                ...
                if agent.discrete_actions:
                    ...
                    action = np.squeeze(cont_actions)
                ...
                memory.save_to_memory(state, cont_actions, reward, next_state, termination)
